...
function activityClaimsExternalOutcome(activity: Activity) {
  return /\b(?:sent|delivered|opened|clicked|(?:social|facebook|instagram|tiktok|twitter|x|youtube|post|reel|video) (?:views?|impressions?|likes?|shares?|comments?|followers?|subscribers?) (?:logged|received|collected|gained|recorded|reported|counted|secured)|(?:views?|impressions?|likes?|shares?|comments?|followers?|subscribers?) (?:logged|received|collected|gained|recorded|reported|counted|secured)|replied|received replies|emailed|mailed|texted|sms sent|messaged|whatsapped|called|phoned|rang|canvassed|door-knocked|dispatched|forwarded|posted|published|broadcast|distributed|launched|went live|live on (?:site|website|social|provider|mailchimp|crm)|shared on social|(?:newsletter|email blast|campaign update|supporter update|mailout|mailing|broadcast) (?:went out|went live)|submitted to (?:council|committee|planning|authority|mp|mps|provider|crm)|provider connected|provider delivery|live delivery|contacts imported|imported contacts|contacts uploaded|uploaded contacts|contacts? (?:exported|downloaded|extracted)|(?:supporter|campaign|contact|voter) list (?:exported|downloaded|extracted)|(?:csv|spreadsheet|voter file) (?:exported|downloaded|uploaded|extracted)|mailing list (?:updated|uploaded|synced|exported|downloaded)|(?:audience|segment|supporters?|contacts?) (?:synced|imported|uploaded|added|updated) to (?:action network|nationbuilder|everyaction|ngp(?:van)?|crm|provider|mailchimp)|(?:added|uploaded|synced|imported) (?:audience|segment|supporters?|contacts?) to (?:action network|nationbuilder|everyaction|ngp(?:van)?|crm|provider|mailchimp)|exported to (?:mailchimp|crm|provider|mailing list)|synced to provider|synced to crm|crm synced|scheduled externally|scheduled with provider|scheduled in provider|provider scheduled|production scheduled|automation (?:started|launched|triggered)|journey (?:started|launched|triggered)|meeting booked|booked meeting|petition signatures? (?:collected|logged|imported)|signatures? collected|(?:supporters?|residents?|people|locals?) (?:signed|added their names? to) (?:the )?petition|petition signed by \d|petition page signatures? (?:received|logged|collected)|(?:webforms?|online forms?|petition forms?|sign-up forms?|signup forms?) (?:submitted|received|collected|logged|filed)|(?:webform|online form|petition form|sign-up form|signup form) submissions? (?:submitted|received|collected|logged|filed)|(?:newsletter|email|mailing list|supporter|campaign) (?:signups?|sign-ups?|opt-ins?|subscribers?) (?:collected|logged|received|imported|added|captured|registered)|(?:signups?|sign-ups?|opt-ins?|subscribers?) (?:collected|logged|received|imported|added|captured|registered)|(?:email|newsletter|mailing) list (?:grew|increased|expanded)|(?:petition|signatures?|objections?|comments?|responses?|representations?|submissions?|letters?|briefings?|deputations?) (?:handed in|handed over|presented|served) (?:to|with|at) (?:council|committee|planning|authority|inspectorate|planning inspectorate|mp|mps|decision-maker)|donations? (?:collected|logged|imported|received)|funds? raised|pledges? (?:collected|logged|secured)|volunteers? (?:recruited|signed up|registered)|(?:events?|rall(?:y|ies)|public meetings?|town halls?) (?:held|hosted|scheduled|booked|announced)|(?:event|rally|meeting|town hall) rsvps? (?:collected|logged|confirmed|imported)|rsvps? (?:collected|logged|confirmed|imported)|(?:survey|poll|form) responses? (?:collected|logged|imported|received)|(?:leaflets?|flyers?|posters?|placards?) (?:printed|ordered|delivered|distributed|posted|dropped)|(?:paid ads?|adverts?|ads?|media buy|ad buy) (?:booked|bought|purchased|launched|started|went live|live|published|funded)|(?:ad spend|media spend|advertising budget) (?:spent|committed|booked)|press (?:coverage|hit|pickup) secured|media coverage secured|(?:press release|media statement|public statement|quote) (?:issued|released)|journalists? (?:contacted|briefed|pitched)|(?:press|media|news) desks? (?:contacted|briefed|pitched)|interviews? booked|(?:councillors?|mps?|decision-makers?|planning officers?|committee members?) (?:contacted|briefed|pitched|lobbied|emailed|met|written to)|(?:contacted|briefed|pitched|lobbied|emailed|met|wrote to) (?:councillors?|mps?|decision-makers?|planning officers?|committee members?)|(?:planning|licensing|council|appeal|consultation) (?:objections?|comments?|responses?|representations?|submissions?) (?:submitted|lodged|filed|logged|registered)|(?:objections?|comments?|responses?|representations?|submissions?) (?:submitted|lodged|filed|logged|registered) (?:with|to) (?:council|committee|planning|authority|inspectorate|planning inspectorate|mp|mps)|(?:planning permission|licen[cs]e|application|redevelopment|appeal) (?:approved|rejected|refused|granted|withdrawn|dismissed|allowed)|(?:council|committee|inspectorate|planning inspectorate|minister|decision-maker) (?:voted|decided|ruled|agreed|committed)\b.{0,80}\b(?:for|against|approve|reject|refuse|grant|withdraw|dismiss|allow)|(?:campaign|case|fight) (?:won|lost|succeeded|defeated)|foi requests? (?:submitted|lodged|filed|logged|registered|sent)|(?:judicial review|legal challenge|court action|injunction) (?:launched|started|filed|issued|lodged|served)|media coverage secured|wrote back(?:\b.{0,80}\b(?:source|campaign factory|public source))?|write-back(?:\b.{0,80}\b(?:source|campaign factory|public source))?|saved to campaign factory|(?:updated|changed|mutated|rewrote|overwrote)\b.{0,60}\b(?:public|canonical|read-only) source document|(?:production|source) data (?:changed|updated|mutated|rewritten))\b/i.test(activity.label);
}
...
function sanitizeStateForWorkspace(state: DemoState, expectedWorkspaceKey: string): DemoState {
  ...
  const activity = state.activity.filter(
    (item) =>
      ...
      !activityClaimsExternalOutcome(item) &&
