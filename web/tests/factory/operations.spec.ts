...
test("operations source API: empty upstream run failures stay source-unavailable before document hydration", async () => {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (String(input).endsWith(`/api/factory/runs/${curatedId}`)) {
      return new Response(null, { status: 500, headers: { "content-length": "0", "cache-control": "public, max-age=0, must-revalidate", date: "Fri, 17 Jul 2026 11:53:48 GMT", "x-matched-path": "/api/factory/runs/[id]", "age": "0", "x-vercel-cache": "MISS", "x-vercel-id": "lhr1::iad1::ops-500" } });
    }
...
test("operations workspace: failed direct source load keeps canonical source brief links", async ({ page }) => {
  ...
  await expect(page.getByText("Source response:")).toBeVisible();
  await expect(page.getByText("upstream HTTP 500 · request lhr1::iad1::direct-500 · matched /api/factory/runs/[id]/documents · cache MISS · cache policy public, max-age=0, must-revalidate · age 0s · source date Fri, 17 Jul 2026 11:53:49 GMT · content length 0 bytes · empty upstream body · no upstream content type")).toBeVisible();
