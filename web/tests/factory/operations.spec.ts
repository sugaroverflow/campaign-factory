...
test("operations source API: empty upstream run failures stay source-unavailable before document hydration", async () => {
  ...
  try {
    ...
    const body = (await response.json()) as { error?: string; detail?: string; sourceOrigin?: string; sourceStep?: string; sourceHttpStatus?: number; sourceRequestId?: string; sourceMatchedPath?: string; sourceCacheStatus?: string; sourceCacheControl?: string; sourceAgeSeconds?: number; sourceContentLength?: number; sourceBodyEmpty?: boolean; sourceContentTypeMissing?: boolean; documents?: unknown[]; sourceRunUnavailable?: boolean };
    expect(body.error).toBe("Campaign source run unavailable");
...
test("operations workspace: failed direct source load keeps canonical source brief links", async ({ page }) => {
  ...
  await expect(page.getByText("Source response:")).toBeVisible();
  await expect(page.getByText("upstream HTTP 500 · request lhr1::iad1::direct-500 · matched /api/factory/runs/[id]/documents · cache MISS · cache policy public, max-age=0, must-revalidate · age 0s · content length 0 bytes · empty upstream body · no upstream content type")).toBeVisible();
